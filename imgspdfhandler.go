...
func (me *ImgsPdfHandler) ServeHTTP(w http.ResponseWriter,r *http.Request){
	...
	for c := 0; c < x; c++ {
		pdf.Image(Path_Www("/imgs/im1.jpg"), float64(10+(c*60)), pdf.GetY()+25, nil)
	}
	...
	for d := 0; d < y; d++ {
		pdf.Image(Path_Www("/imgs/im2.jpg"), float64(10+(d*50)), pdf.GetY()+65, nil)
	}

	for c := 0; c < x; c++ {
		if c % 2 == 0 {
			pdf.Image(Path_Www("/imgs/im1.jpg"), float64(10+(c*60)), pdf.GetY()+105, nil)
		} else{
			pdf.Image(Path_Www("/imgs/im2.jpg"), float64(10+(c*60)), pdf.GetY()+105, nil)
		}
	}
